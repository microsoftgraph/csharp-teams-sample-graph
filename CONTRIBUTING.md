...
4.	The last step involves copying your fork to your local machine. To do this go open GitBash. On the command prompt enter:
		...
		git clone https://github.com/{your user name}/<projectname>.git

   Next create a reference to the root repository by entering these commands:

		cd <projectname>
		git remote add upstream https://github.com/OfficeDev/<projectname>.git
		git fetch upstream
...
You can now navigate to the repository on your local machine using Windows Explorer. The repository files are in `C:\Users\<yourusername>\<projectname>`.
...
4.	Ensure that the Base branch is `OfficeDev/<projectname>@master` and the Head branch is `<your username>/<projectname>@<branch name>`
...
One of the site administrators will now process your pull request.  Your pull request will surface on the OfficeDev/<projectname> site under Issues.  When the Pull Request is accepted, the issue will be resolved.
...
For example, suppose your local branch X was successfully merged into the OfficeDev/<projectname> master branch and you want to make further updates to the content that was merged. Create a new local branch, X2, from the OfficeDev/<projectname> master branch. To do this, open GitBash and execute the following commands:
...
...and verifying the content. (The `checkout` command updates the files in `C:\Users\<yourusername>\<projectname>` to the current state of the X2 branch.) Once you have checked out the new branch, you can make updates to the content and commit them as usual. However, to avoid working in the merged branch (X) by mistake, it is best to delete it (see the following "Delete a Branch" section).
